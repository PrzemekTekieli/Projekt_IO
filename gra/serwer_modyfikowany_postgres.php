...
function wsOnMessage($clientID, $message, $messageLength, $binary) {
    ...
    if($pieces[0] == "log" && $pieces[1] != "" && $result->num_rows == 1) { // zaloguj
        ...
        if($row["haslo"] == $pieces[2]) {
            ...
            $player = $pieces[1];
			$sql = "select nazwa from Postacie where login = (select login from Gracze WHERE login = '$pieces[1]')";
			$result = $conn->query($sql);
        ...
        $sql3 = "select login, nazwa from Postacie join Gracze on Gracze.login = Postacie.login";
        ...
        if($jest && $gracz == $player) {
            ...
            $x = 0;
            $y = 0;
			$sql = "select max(id_statystyki) as max_stat from Statystyka";
			$result = $conn->query($sql)->fetch_assoc();
			$id_stat = $result["max_stat"] + 1;
            $sql = "insert into Statystyka (id_statystyki, atak, obrona, hp) values ($id_stat, 30, 30, 100)";
			$conn->query($sql);
            $sql = "select id_gracza FROM gracze WHERE login=$player";
            $player_id = $conn->query($sql);
            $sql = "insert into Postacie (id_postaci, id_lokacji, id_statystyki, nazwa) values ('$player_id', 1, '$id_stat', '$postac')";
            $conn->query($sql);
...
function wejscieDoLokacji($conn, $x, $y, $postac, $clientID) {
	...
	if($result->num_rows > 0) {
	    ...
	    if($row["zleceniodawca"]) {
	        $sql = "select m.*, p.nazwa from Misje m join Potwory p on id_potwora_do_zabicia=id_potwora";
	        $result = $conn->query($sql)->fetch_assoc();
